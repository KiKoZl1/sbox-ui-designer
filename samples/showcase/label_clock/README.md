...
- [`label_simple`](../label_simple/) — even smaller: a static label with no binding at all. Good first read if `OnChange` propagation is new.
- [`counter_button`](../counter_button/) — adds an event handler and a TwoWay-style write-back to the same OneWay foundation shown here.
- [`progress_bar`](../progress_bar/) — same OneWay/OnChange shape, but driving a numeric `Width` property instead of a text label.
