...
|---|---|---|---|---|
| _None_ | — | — | — | This sample declares no SUI Variables. All toast state lives controller-side in `_active` (List<ActiveToast> with per-toast `SpawnedAt`/`DismissAt`/`RemoveAt` timestamps) and `_pendingSpawns` (Queue of `(kind, title, body)` tuples drained on `OnUpdate`). The only `.sui` element exposed for runtime mutation is the `ToastsContainer` Panel (`ExposeAsVariable: true`), which the controller `AddChild`s into directly. |
...
No element has any binding — toasts are created and styled imperatively via `AddChild<Panel>` / `AddClass` / `Style.*` inside the controller. The intro/outro animations are CSS class flips (`.show`, `.dismiss`) rather than Variable-driven property writes.

## Events

| Element | Event | Mode | Handler |
|---|---|---|---|
| `InfoButton` | `OnClick` | Code | `OnInfoClick` |
| `SuccessButton` | `OnClick` | Code | `OnSuccessClick` |
| `WarningButton` | `OnClick` | Code | `OnWarningClick` |

All three handlers must be assigned on the wrapper **before** `Hud.Show()` — `SyncFieldsTo` copies the Action delegates into the rendered Panel during `Show`, so assigning afterward leaves the Panel-side fields null and clicks no-op. Each handler just enqueues a `(kind, title, body)` tuple into `_pendingSpawns` and bumps the per-kind line cycler; the actual `AddChild` work happens next frame in `OnUpdate` to stay outside Sandbox.UI's event dispatch loop (mutating children inside `onclick` throws `IndexOutOfRangeException` deep in `Panel` — same gotcha called out in `dialog_system` and `drag_drop_inventory`). The per-toast click-to-dismiss `onclick` is wired at runtime inside `SpawnToast` via `AddEventListener`, not declared in the `.sui`.

## File map

| File | Role |
|---|---|
| `notification_toast_queue.sui` | Designer source — defines the three trigger buttons + the empty `ToastsContainer` Panel that hosts spawned toasts. |
| `NotificationToastQueuePanel.razor` | Generated by SUI Designer on Compile. Do not hand-edit. |
| `NotificationToastQueuePanel.scss` | Generated base styles. Toast-specific rules go in `User.scss`. |
| `NotificationToastQueuePanel.User.scss` | Hand-authored CSS for `.toast`, `.toast.show`, `.toast.dismiss`, `.toast-icon`, kind tints, and the 0.3 s `transition`. See **Required `User.scss` rules** below. |
| `NotificationToastQueue.cs` | Generated wrapper (`Code/Samples/NotificationToastQueue/`, namespace `Sandbox.Samples`). Exposes the `ToastsContainer` @ref and the three `OnXxxClick` Action delegates. |
| `NotificationToastQueueController.cs` | Hand-authored. Wires the three click delegates in `OnStart`, drains `_pendingSpawns` and ticks per-toast lifecycles in `OnUpdate`, and owns `SpawnToast` / `DismissImmediate`. |
