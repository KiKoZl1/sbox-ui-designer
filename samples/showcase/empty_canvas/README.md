...
- [`counter_button`](../counter_button/) — the natural next sample. Adds one Variable, one OneWay binding, and one OnClick event to the same skeleton.
- [`dialog_system`](../dialog_system/) — what a "real" HUD built on top of these primitives looks like once Events and TwoWay bindings enter the picture.
- [`settings_full`](../settings_full/) — the far end of the difficulty curve, showing every binding mode and input widget exercising the same mount/teardown contract used here.
