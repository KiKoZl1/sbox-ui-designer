...
4. Press **Play**. The card appears in the bottom-left. Click the input, type something, click **Send** (or press **Enter**), and watch the message render. Try `/me waves` and `/system maintenance window 22:00 UTC` for the two command flavours.
...
Pressing **Enter** inside the focused TextEntry triggers a send via the engine's native `"onsubmit"` event. The controller registers the listener in `OnUpdate` once `Hud.View?.ChatInputRef` becomes non-null (the engine captures `@ref` after the first paint — binding in `OnStart` would no-op). One-shot guard flips after wire-up so the check only costs an `if` per frame for one frame.
...
- `OnUpdate` runs a one-shot that wires the TextEntry's `"onsubmit"` event to `OnSendClick` once `Hud.View?.ChatInputRef` becomes non-null (engine captures the `@ref` after first paint, so this can't happen in `OnStart`). After wire-up the flag flips and the check no-ops.
- `OnSendClick` runs `Hud.Apply.All()` to flush the Manual TextEntry, trims the draft, exits if empty, parses `/me` and `/system` command prefixes, appends a `ChatMessage`, trims the list to `MaxMessages`, calls `RenderMessages()`, and clears the input by writing `""` directly to `Hud.View.ChatInputRef.Text` (the wrapper `Hud.ChatInputText = ""` alone doesn't push back to the widget on re-render).
...
Surprises specific to this sample — most are codegen quirks fixed in v1.5, plus the universal first-compile gotcha.

| Symptom | Cause | Fix |
|---|---|---|
| Caret is invisible inside the TextEntry — you can type but see no blinking cursor on the dark input bar. | Engine reads `ComputedStyle.CaretColor` from the wrapper Style, not from the inner `.content-label`. Without an explicit caret-color on the wrapper it falls back to `Color.Black`, which disappears against the `#1f2937` input strip. | Set the TextEntry's `CaretColor` field in the `.sui` (any visible colour — white or the green accent works). Designer v1.5+ codegen emits it onto the wrapper selector. Requires `sbox-ui-designer` commit `d3d4459` or later. |
| Runtime-added message rows stack horizontally instead of vertically, overflowing the `MessageList` to the right. | The codegen used to gate `flex-direction` emission on `Layout.Mode = Flex`. `MessageList` is `Mode = Absolute` so `FlexDirection = Column` was silently dropped, leaving the default row direction in the rendered SCSS. | Pull `sbox-ui-designer` commit `0a514ee` or later — codegen now emits `flex-direction` whenever it is set, regardless of Layout.Mode. Re-Compile `chat_panel.sui` to regenerate the SCSS. |
| Welcome message does not appear on mount — the panel renders empty and the first message only shows up after you click Send once. | `RenderMessages()` was called from `OnStart`, but `Hud.View?.MessageList` is `null` until the first paint, so the call no-ops silently. The `MessageCountText` Variable still updates because it is bound, but the runtime `AddChild<Label>()` calls hit a null Panel. | Move the seed render into a one-shot guard inside `OnUpdate` that fires once `Hud.View?.MessageList` becomes non-null. The controller in this sample already does this — copy the `_seeded` flag pattern if you fork. |
