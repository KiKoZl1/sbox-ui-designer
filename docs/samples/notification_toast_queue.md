---
layout: default
title: notification_toast_queue
parent: Samples
nav_order: 12
permalink: /samples/notification_toast_queue/
---

# notification_toast_queue
{: .no_toc }
...
2. Add `NotificationToastQueuePanel.User.scss` (see [Required `User.scss` rules](#required-userscss-rules) below).
...
- **`OnUpdate`** runs four passes each frame:
  1. Bootstrap check for `ToastsContainer` capture.
  2. **Promote pass first** — adds the `.show` class to any toast missing it, triggering the CSS intro transition. This MUST come before draining new spawns.
  3. Drain `_pendingSpawns` queue and `SpawnToast` each — so newly-spawned toasts stay in the off-screen `.toast` state for one paint cycle before getting `.show` on the next frame.
  4. Auto-dismiss tick — adds `.dismiss` class when `DismissAt` passes, `Delete()`s the panel when `RemoveAt` passes.
...
The CSS transition needs at least one paint of the initial `.toast` state (`opacity: 0`, `translateX` off-screen) before `.show` flips to the visible state. If you drain-then-promote in the same frame, the engine paints once with `.toast.show` already applied, the transition has no "from" state to animate from, and the toast pops in instantly. Promoting first means toasts spawned this frame paint as `.toast` (off-screen) at frame end, and next frame's promote pass adds `.show` — the engine sees both states across frames and the slide-in animates.
...
| File | Purpose |
|---|---|
| `notification_toast_queue.sui` | Designer document — root canvas, `ToastsContainer` (top-right, `FlexDirection: Column`, `Gap: 10`, exposed as `@ref`), three trigger buttons (Info/Success/Warning) wired to Code-mode `OnInfoClick`/`OnSuccessClick`/`OnWarningClick`. |
| `NotificationToastQueueController.cs` | Component that owns `Hud`, drains the spawn queue, ticks per-toast timers, and handles `AddChild<Panel>` for each toast structure. |
| `NotificationToastQueuePanel.User.scss` | Required transitions, layout rules, and per-kind icon colours. Without it, toasts spawn instantly with no animation. |

The wrapper (`NotificationToastQueue` partial class) is generated by the Designer. It exposes `View.ToastsContainer` (the `@ref` capture) and the three `Action` delegate slots (`OnInfoClick`, `OnSuccessClick`, `OnWarningClick`).
...
|---|---|---|
| Toast pops in instantly with no slide animation | Drain-then-promote order: the spawn and `.show` class apply in the same frame, so the engine never paints the off-screen `.toast` state. | Keep the promote pass BEFORE the spawn drain in `OnUpdate` — see [Why pass order matters](#why-pass-order-matters). |
| Clicks do nothing | `OnXxxClick` delegates were assigned after `Hud.Show()`, so `SyncFieldsTo` copied `null` into the renderer. | Assign all three `Hud.OnXxxClick` delegates BEFORE calling `Hud.Show( GameObject, ... )` in `OnStart`. |
| `IndexOutOfRangeException` when clicking a toast | You're calling `toast.Delete()` directly inside the `onclick` lambda — mutates the engine's child list during its own event dispatch. | Use `DismissImmediate( active )` which only adjusts the `DismissAt` timestamp; `OnUpdate` does the actual `Delete()` next frame. Same gotcha as `dialog_system` and `drag_drop`. |
| Toasts overlap instead of stacking with a gap | `ToastsContainer` is `FlexDirection: Row` or has `Gap: 0`. | Set `FlexDirection: Column`, `Gap: 10` on the `ToastsContainer` element in the `.sui`. |
| Outro animation cuts off mid-slide | `OutroDuration` `[Property]` doesn't match the `transition` duration in `User.scss`. | Both must be 0.3 s by default. If you change one, change the other. |
| Toasts pile up off-screen at the bottom | Container `Height` is too small or the parent is clipping (`Overflow: Hidden`). | The reference `.sui` sets `Width: 340, Height: 720` with `Overflow: Visible` — stack cap (see [Extending it](#extending-it)) is the proper fix for production. |
| `Hud.View?.ToastsContainer` stays `null` forever | `ExposeAsVariable` flag wasn't set on the container element in the Designer, so no `@ref` was emitted. | Open the `.sui`, select `ToastsContainer`, toggle `Flags.ExposeAsVariable = true`, recompile. |

---

## See also

- [Source on GitHub](https://github.com/KiKoZl1/sbox-ui-designer/tree/main/samples/showcase/notification_toast_queue)
- [Showcase samples]({% link reference/showcase-samples.md %}) — the full landing page with all 16 samples and their Variables / Bindings tables inlined.
- [Sample index]({% link reference/sample-index.md %}) — short catalog of every shipped sample.
- [`dialog_system`]({% link samples/dialog_system.md %}) — same "mutate children outside the dispatch loop" pattern for modal flow.
- [`drag_drop_inventory`]({% link samples/drag_drop_inventory.md %}) — the canonical example of the IOOR gotcha and the queue-then-process workaround.
- [`chat_panel`]({% link samples/chat_panel.md %}) — another spawn-and-fade child-list pipeline, this time for scrolling chat messages.
