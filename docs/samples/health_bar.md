...
- [Sample index]({% link reference/sample-index.md %}) — quick reference table of every sample.
- [Bindings]({% link concepts/bindings.md %}) — full docs on OneWay / TwoWay / OneTime modes and `UpdateTrigger`.
- [Wrapper generation]({% link concepts/wrapper-generation.md %}) — how `Manual` Variables become auto-properties on the generated wrapper.
